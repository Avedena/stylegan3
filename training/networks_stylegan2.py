    ...
    def forward(self, x, w, noise_mode='random', fused_modconv=True, gain=1):
        ...
        if self.use_noise and noise_mode == 'random':
            if self.square:
                ...
                noise = torch.randn([x.shape[0], 1, self.resolution, self.resolution // 2],
                                    device=x.device) * self.noise_strength
    ...
    def __init__(self,
        ...
        if in_channels != 0:
            self.conv0 = SynthesisLayer(in_channels, out_channels, w_dim=w_dim, resolution=resolution, up=2,
                                        ...
                                        channels_last=self.channels_last, square=square, **layer_kwargs)
            self.num_conv += 1

        self.conv1 = SynthesisLayer(out_channels, out_channels, w_dim=w_dim, resolution=resolution,
                                    conv_clamp=conv_clamp, channels_last=self.channels_last, square=square,
                                    **layer_kwargs)
        self.num_conv += 1
    ...
    def __init__(self,
        ...
        for res in self.block_resolutions:
            ...
            block = SynthesisBlock(in_channels, out_channels, w_dim=w_dim, resolution=res,
                                   img_channels=img_channels, is_last=is_last, use_fp16=use_fp16, square=square,
                                   **block_kwargs)
...
class Generator(torch.nn.Module):
    def __init__(self,
        ...
        self.synthesis = SynthesisNetwork(w_dim=w_dim, img_resolution=img_resolution, img_channels=img_channels,
                                          square=square, **synthesis_kwargs)
    ...
    def __init__(self,
        ...
        for res in self.block_resolutions:
            ...
            block = DiscriminatorBlock(in_channels, tmp_channels, out_channels, resolution=res,
                                       first_layer_idx=cur_layer_idx, use_fp16=use_fp16, square=square, **block_kwargs,
                                       **common_kwargs)
            setattr(self, f'b{res}', block)
        ...
        if c_dim > 0:
            self.mapping = MappingNetwork(z_dim=0, c_dim=c_dim, w_dim=cmap_dim, num_ws=None, w_avg_beta=None,
                                          **mapping_kwargs)
        self.b4 = DiscriminatorEpilogue(channels_dict[4], cmap_dim=cmap_dim, resolution=4, square=square,
